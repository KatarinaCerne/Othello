...
##        mozne_poteze = poteze_igralca - poteze_nasprotnika
    ...
    def povleci(self, poteza, canvas = None, zetoni = None):
        ...
        self.prejsnja_stanja.append([self.na_potezi, self.stejcrne, self.stejbele, self.vrednost_crnih, self.vrednost_belih, polje])
...
##            elif (i, j) in DOBRA: self.vrednost_belih+=7
    ...
    def preklici(self, poteza):
        ...
        self.na_potezi, self.stejcrne, self.stejbele, self.vrednost_crnih, self.vrednost_belih, self.polje = self.prejsnja_stanja.pop()
    ...
    def odigraj(self, i, j):
        ...
        if self.igra.polje[i][j] is None and je_veljavna:
            ...
            if r == "Neodločeno":
                ...
                if ((self.igra.na_potezi == CRNI and self.crni == "računalnik") or
                    (self.igra.na_potezi == BELI and self.beli == "računalnik")):
...
##    def pomoc(self):
##        if (self.igra.na_potezi == CRNI and self.crni == "človek") or (self.igra.na_potezi == BELI and self.beli == "človek"):
##            for (i, j) in self.igra.poteze(self.igra.na_potezi):
##                self.canvas.create_oval(i*50+20, j*50+20, i*50+25, j*50+25, fill="yellow")
##                self.pike[i][j] = self.canvas.create_oval(i*50+20, j*50+20, i*50+25, j*50+25, fill="yellow")
